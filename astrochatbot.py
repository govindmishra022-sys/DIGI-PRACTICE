...
with st.expander("⚙️ Settings & Configuration", expanded=True):
    ...
    with col_api:
        ...
        user_api_key = st.text_input("Enter Gemini API Key", type="password", placeholder="AIzaSy...")
        if user_api_key:
...
with st.expander("📜 Your North Indian Kundli Chart"):
    ...
    st.markdown(generate_svg_kundli(sun_sign, moon_sign), unsafe_allow_html=True)
